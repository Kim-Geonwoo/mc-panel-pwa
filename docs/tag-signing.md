...
서명됩니다. 릴리스는 서명된 소스 태그와, 그 태그에서 빌드되어 cosign으로 서명되는
GHCR 컨테이너 이미지로 구성됩니다(아래 참조). GitHub 릴리스에 별도 바이너리
자산은 첨부하지 않습니다.
...
maintainer's dedicated SSH key (Ed25519). A release consists of the signed
source tag plus a GHCR container image built from it and signed with cosign
(see below). No separate binary assets are attached to GitHub releases.
