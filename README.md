...
성능 차트, 게임 ↔ 디스코드 ↔ 웹을 잇는 3방향 채팅에 더해, **서버 주도 UI(SDUI)** 기반의
페이지 구성을 제공합니다 — 레이아웃 JSON 하나로 재빌드 없이 서버마다 패널 화면을 구성합니다.
정적으로 익스포트한 Next.js 프런트엔드를, 외부 의존성이 없는 단일 Go 바이너리가 서빙합니다.
...
- **화면 구성(SDUI)** — 서버가 내려주는 레이아웃 JSON(`GET /api/layout`)으로 탭 구성·순서·
  라벨·타이틀·테마·화면 블록 배치를 재빌드 없이 변경합니다. 화이트리스트 블록 레지스트리와
  재귀 렌더러가 미지 타입·손상 입력에도 안전하게 동작합니다(폴백·부분 실패 격리).
- **스튜디오(시각 편집기)** — 관리자 전용 `/studio`에서 팔레트·라이브 프리뷰 캔버스·구조
  트리(드래그앤드롭)·인스펙터로 화면을 시각 편집하고 발행합니다. 드래프트 자동저장,
  실행취소/재실행, 발행 전 스키마·상한 사전검사. 관리자 로그인은 `PANEL_ADMIN_CODE`.
...
| PUT | `/api/layout` | Bearer(관리자) | 레이아웃 발행 — 스키마 검증·세션당 분당 10회 제한·감사 로그. 데모 모드에선 403. 루프백 리스너에선 무인증 사용 가능(도구용) |
...
- [x] 웹 UI 단위 테스트 도입(vitest — 스키마·렌더러·탭 구성; Go API는 문장 커버리지 ~82%)
- [x] 드래그앤드롭 시각 편집기(`/studio`) — 팔레트·라이브 프리뷰·구조 트리 DnD·발행
- [ ] README 스크린샷 추가
